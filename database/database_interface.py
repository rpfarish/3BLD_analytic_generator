...
def main():
    ...
    label = main_font.render(
        f"Put a ',' between the pair and number", True, (255, 255, 255)
    )
    bfs_b = Button(
        WIN,
        light_gray,
        ...
        win_w=WIDTH,
        win_h=HEIGHT,
        text="Insert/update pair in database",
        font_size=15,
        button_value="BFS",
    )
    ...
    def redraw_window():
        ...
        bfs_b.draw_resize(WIDTH, HEIGHT, -0.05, 0.2)
    ...
    while True:
        ...
        if input_box1.text.count(",") == 1:
            ...
            inserted = bfs_b.call_func(
                insert_pair, can_insert, "edges_cycle_averages", a, b
            )
            ...
            if r is not None and a in dict(r):
                label = main_font.render(
                    f"'{input_box1.text}' is in the database!!", True, (255, 255, 255)
                )
            else:
                label = main_font.render(
                    f"'{input_box1.text}' is not in the database!!",
                    True,
                    (255, 255, 255),
                )
