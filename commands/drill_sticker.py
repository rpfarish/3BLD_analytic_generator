...
def drill_sticker(args: list, buffers):
    """Drills a list of letterpairs from text file until all pairs are drilled.
    ...
    -r Randomly generates scrambles indefinitely and does not removed drilled letterpairs
    -f Specifies the number of cycles to be included in a scramble (Recommended 2).
        Defaults to 3 and will reduce as cases decrease.

    In the text file, enter a pair as A* to drill all pairs that start with A (or *A for inverse).
    A letterpair in quotes signifies a pair that was already drilled has occurred again.
